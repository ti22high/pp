...
// Применяет область обрезки к изображению. cropFrac — нормализованный
// прямоугольник [0..1] в координатах исходной картинки; box — новый
// display-bbox фигуры (slide-coords). Композиция кропов: новый кроп задаётся
// относительно ТЕКУЩЕГО кропа, поэтому пересчитываем в абсолютные доли.
export function applyImageCrop(
  ...
  useDeckStore.setState((state) => {
    ...
    const prev = sh.crop ?? { x: 0, y: 0, w: 1, h: 1 };
    // Композиция: новые доли вложены в предыдущий кроп.
    sh.crop = {
      x: prev.x + cropFrac.x * prev.w,
      y: prev.y + cropFrac.y * prev.h,
      w: cropFrac.w * prev.w,
      h: cropFrac.h * prev.h,
    };
    sh.x = box.x;
...
export function resetImageCrop(slideId: string, shapeId: string): void {
  useDeckStore.setState((state) => {
    ...
    if (!slide) return;
    const sh = slide.shapes.find((x) => x.id === shapeId);
    if (!sh || sh.type !== 'image' || !sh.crop) return;
    // Текущий display показывает crop.w*natW при ширине sh.w → масштаб на доле.
    const sx = sh.w / sh.crop.w;
    const sy = sh.h / sh.crop.h;
    sh.w = sx;
    sh.h = sy;
    sh.crop = undefined;
    state.deck.modifiedAt = new Date().toISOString();
