...
- [x] 3.21. WordArt mini-editor: новый тип `wordart` (text + font + fill + outline, fill/stroke из baseShape). Кнопка «WordArt» на тулбаре → вставка + диалог-редактор (текст/шрифт/размер/жирный-курсив/заливка/контур), двойной клик открывает редактор. Рендер Konva.Text с обводкой (fillAfterStroke), текст тянется под bbox при resize.
