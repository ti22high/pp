...
- [ ] 3.11e. Rich-текст в ячейках таблицы: цвет текста, шрифт, размер, жирный/курсив (сейчас текст ячейки — простая строка). Нужно для полноценного форматирования И для переноса стилей текста при вставке из Excel/Sheets (сейчас переносятся только фон+выравнивание). Пересекается с 3.34 (панель форматирования текста). (Запрос пользователя 2026-05-20.)
