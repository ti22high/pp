...
- [x] 3.21. WordArt mini-editor: новый тип `wordart` (text + font + fill + outline, fill/stroke из baseShape). Кнопка «WordArt» → вставка + инлайн-правка текста прямо на холсте (как в Word, через editingShapeId/WordArtOverlay), двойной клик правит на месте. Стиль (шрифт/размер/жирный-курсив/заливка/контур) — в контекстном тулбаре сверху (WordArtContextToolbar), live. Рендер Konva.Text с обводкой (fillAfterStroke), текст тянется под bbox при resize.
- [x] 3.22. Bookmarks: поле `bookmark` (имя-якорь) на любой фигуре — секция «Закладка» в инспекторе; bookmarkId = id фигуры. В hyperlink-диалоге режим «На закладку» со списком всех закладок деки (lib/bookmarks.listBookmarks). Навигация по ссылке — Phase 6 (presentation).
- [x] 3.23. Speaker notes panel (TipTap): нижняя панель «Заметки докладчика» правит slide.notes активного слайда (хранится строкой = сериализованный TipTap JSON, lib/notes.parseNotesContent с поддержкой legacy-текста). Сворачивается в полоску (ui.showNotes/toggleNotes), grid-area notes под канвасом. Показ заметок — в Presenter View (Phase 6).
- [x] 3.24. KaTeX equation insert: тип `equation` (latex). Кнопка «Формула» + двойной клик → диалог LaTeX с live-превью KaTeX и примерами. Рендер DOM-оверлеем (§6.6, EquationOverlay, pointer-events:none) + прозрачный Konva-прокси (EquationShapeView) для выделения/перетаскивания/масштаба. Ограничения оверлея — DECISIONS 2026-05-21.
- [x] 3.24a. Визуальный ввод формул «как в Word»: добавлен MathLive (`<math-field>`, MathField.tsx) — WYSIWYG-редактирование, на выходе LaTeX. Палитра кнопок-шаблонов (структуры/греческие/операторы) вставляет в поле через mf.insert с плейсхолдерами `#?`. Сырой LaTeX — по кнопке-тогглу снизу. Рендер на слайде — KaTeX. Шрифты офлайн через mathlive/fonts.css. (DECISIONS 2026-05-21.)
- [x] 3.24b. Рукописный ink-ввод формул (офлайн, посимвольно): распознаватель жестов $P (`lib/inkRecognizer.ts`, point-cloud, мультиштриховый, без зависимостей) + библиотека ~41 шаблона символ→LaTeX (`lib/inkTemplates.ts`). UI: тоггл «Рисовать символ» в диалоге формулы → холст (`InkPad.tsx`), рисуешь стилусом/мышью → top-4 вариантов LaTeX → вставка в MathLive. Полное layout-распознавание целого уравнения — вне MVP (нужен cloud/коммерч.), см. DECISIONS 2026-05-27.
