...
- [ ] 3.14i. Продвинутые оси (Excel/PP-уровень): тип оси значений — линейная / **логарифмическая**; ручные min/max и шаг делений (major/minor); ось категорий как **ось дат** (парсинг дат, масштаб по времени, интервалы день/месяц/год); **вторичная ось Y**; единицы (×1000/млн), угол подписей. Оси рисуем сами на Konva — всё реализуемо, но объёмно. Хранить параметры осей в chartShape (axisX/axisY: {type, min, max, majorUnit, logBase…}). (Запрос пользователя 2026-05-20.)
