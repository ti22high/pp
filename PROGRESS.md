...
- [ ] 3.11d. Alt-текст на отдельные ячейки таблицы: поле altText в схеме ячейки + «Alt-текст ячейки…» в контекстном меню (по выбранной ячейке/диапазону) + показ при наведении на ячейку. Alt на ВСЮ таблицу уже работает (общая секция Alt-текст для любой фигуры). Редкий кейс (в PowerPoint alt только на таблицу) — отдельной задачей. (Запрос пользователя 2026-05-20.)
...
- [ ] 3.14f. UX редактирования графика: вынести управление из плавающей панели в КОНТЕКСТНЫЙ ТУЛБАР СВЕРХУ (появляется при выделенном графике: тип, легенда, сетка, «Данные…», заголовок) + правка прямо на графике (двойной клик по подписи/оси/легенде → инлайн-редактирование, двойной клик по серии → её настройки). Пользователь (2026-05-20): текущее редактирование в боковой/плавающей панели неудобно, надо «как в PP — сверху и на самом графике». Часть редизайна ленты (Backlog B2); по графикам выделено отдельно. Также сюда: показать по умолчанию легенду/оси и довести «базовый набор PP» (см. 3.14b — заголовок, позиция легенды, data labels, формат чисел осей).
